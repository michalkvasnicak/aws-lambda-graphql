...
function createSNSEventProcessor({
  connectionManager,
  schema,
  subscriptionManager,
}: Options): SNSHandler {
  return async function processSNSEvents({ Records }) {
    for (const record of Records) {
      // process only INSERT events

      // now construct event from dynamodb image
      const event: ISubscriptionEvent = JSON.parse(record.Sns.Message)
...
export { createSNSEventProcessor };
export default createSNSEventProcessor;
